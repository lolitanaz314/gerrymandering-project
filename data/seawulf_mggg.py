...
class Seawulf:
    def __init__(self, precincts_df, state,
        ...
        self.seawulf_districts = pd.DataFrame(
            columns=['districtId', 'districtPlanId', 'stateId', 'total_pop', 'white', 'af_amer', 'asian', 'hispanic', 'native_hawaiian', 'two_or_more',
    ...
    def process_iteration(self, partition, id) -> (pd.DataFrame, pd.DataFrame):
        ...
        for node in partition.graph.nodes:
            ...
            stateId = self.state # partition.graph.nodes[node]['stateId']
            ...
            precincts.append([precinctId, districtId, stateId, total_pop, white, af_amer, asian, native_hawaiian,
                              hispanic, two_or_more, democraticPres, republicanPres,
                              democraticSen, republicanSen, geometry])
        
        precincts = pd.DataFrame(precincts,
                                 columns=['precinctId', 'districtId', 'stateId', 'total_pop', 'white', 'af_amer', 'asian', 
        ...
        districts_info = precincts[
            ['districtId', 'stateId', 'total_pop', 'white', 'af_amer', 'asian', 'hispanic', 'native_hawaiian', 'two_or_more',
...
if __name__ == '__main__':
    ...
    if "Unnamed: 0" in precincts_gdf.columns:
    ...
    output_path = '/Users/cherrypi/Desktop/gerrymandering-project/data/post-processing/postseawulf_CSV_path/'
